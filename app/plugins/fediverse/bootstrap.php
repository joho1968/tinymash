    ...
    $plugins->registerEditorTab(
        ...
        [
            ...
            'renderer' => static function( array $context = [] ) use ( $fediverse_service, $plugins, $plugin_key, $escape, $fediverse_readiness_url ) : string {
                ...
                $html .= '<div class="col-12 col-lg-4">Truncation: <span class="fw-semibold text-body" id="tm-editor-fediverse-truncation-state">' . ( ! empty( $preview['will_truncate'] ) ? 'Yes' : 'No' ) . '</span></div>';
                $html .= '</div>';
                $html .= '<div class="mt-3">';
                ...
                $html .= '<script>(function(){'
                    ...
                    . 'function render(payload){const preview=payload&&payload.preview&&typeof payload.preview==="object"?payload.preview:{};const warnings=Array.isArray(payload&&payload.warnings)?payload.warnings:[];const badgeState=badge(String(payload&&payload.state||"blocked"));const instanceUrl=String(payload&&payload.account_settings&&payload.account_settings.instance_url||"");const message=String(payload&&payload.message||"");readinessBadge.className="badge "+badgeState[0];readinessBadge.textContent=badgeState[1];previewField.textContent=String(preview.status_text||"");characterCount.textContent=String(Number.parseInt(preview.character_count||0,10)||0)+" / "+String(Number.parseInt(preview.character_limit||500,10)||500);characterLimit.textContent=String(Number.parseInt(preview.character_limit||500,10)||500);linkBackState.textContent=preview.include_link_back?"Included":"Off";truncationState.textContent=preview.will_truncate?"Yes":"No";readinessMessage.textContent=message;statusText.textContent=message;statusAlert.className="alert mb-0 "+(payload&&payload.eligible?"alert-success":"alert-secondary");statusUrl.textContent=instanceUrl;statusUrlWrap.classList.toggle("d-none",instanceUrl==="");if(warnings.length===0){warningsList.innerHTML="<li>No current rendering warnings.</li>";return;}warningsList.innerHTML=warnings.map((warning)=>"<li>"+String(warning).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")+"</li>").join("");}'
