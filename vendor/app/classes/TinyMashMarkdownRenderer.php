...
class TinyMashMarkdownRenderer {
    ...
    protected const EMOJI_SHORTCODE_PATTERN = '(?<![A-Za-z0-9]):[a-z0-9_+-]+:(?![A-Za-z0-9])';
    ...
    public function replaceEmojiShortcodes( string $text ) : string {
        if ( $text === '' ) {
            return( '' );
        }

        $replaced_text = preg_replace_callback(
            '/' . self::EMOJI_SHORTCODE_PATTERN . '/u',
            function( array $match ) : string {
                $shortcode = strtolower( (string) ( $match[0] ?? '' ) );
                return( $this->emoji_shortcode_map[$shortcode] ?? $shortcode );
            },
            $text
        );

        return( is_string( $replaced_text ) ? $replaced_text : $text );
    }
    ...
    protected function replaceInlineTokensInText( string $text, bool $classic_smileys_enabled ) : ?string {
        ...
        $pattern = '/(==([^=\r\n][^=\r\n]*?)==|' . self::EMOJI_SHORTCODE_PATTERN . ( $classic_smileys_enabled ? '|' . $this->getSmileyPattern() : '' ) . '|\p{Extended_Pictographic}(?:\x{FE0F}|\x{200D}\p{Extended_Pictographic})*)/u';
